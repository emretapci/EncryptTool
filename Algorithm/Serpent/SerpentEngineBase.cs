...
namespace Encrypt.Algorithm.Serpent
{
    public abstract class SerpentEngineBase : SymmetricAlgorithm, ICryptoTransform
    {
        protected static readonly int _BlockSize = 16;
		...
		{
			byte[] ib = new byte[Math.Max(inputCount, _BlockSize)];
			Array.Copy(inputBuffer, inputOffset, ib, 0, inputCount);

			return ProcessBlock(ib, 0, outputBuffer, outputOffset);
		}
		...
		public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
		{
			...
			byte[] ob = new byte[_BlockSize];
			ProcessBlock(ib, 0, ob, 0);
			return ob;
		}
        ...
        public int ProcessBlock(byte[] input, int inOff, byte[] output, int outOff)
        {
            ...
            Check.DataLength(input, inOff, _BlockSize, "input buffer too short");
            Check.OutputLength(output, outOff, _BlockSize, "output buffer too short");
