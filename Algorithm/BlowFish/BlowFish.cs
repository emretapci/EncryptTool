...
namespace Encrypt.Algorithm.BlowFish
{
	internal class BlowFish : SymmetricAlgorithm, ICryptoTransform
	{
		...
		#region SymmetricAlgorithm implementation
		...
		public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
		{
			...
			byte[] ob = encrypt ? Encrypt_CBC(ib) : Decrypt_CBC(ib);
			Array.Copy(ob, 0, outputBuffer, outputOffset, ob.Length);
		...
		public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
		{
			byte[] ib = new byte[inputCount];
			Array.Copy(inputBuffer, inputOffset, ib, 0, inputCount);
			return encrypt ? Encrypt_CBC(ib) : Decrypt_CBC(ib);
		}
